...
)
{
    ...
    forAll(faceCells, facei)
    {
        ...
        const scalar omegaVis = w*6*nuw[facei]/(beta1_*sqr(y[facei]));
        const scalar omegaLog = w*sqrt(k[celli])/(Cmu25*kappa*y[facei]);

        if (yPlus < 1)
        {
            scalar op = 0.9*exp(-yPlus/0.1) + 0.1;

            omega0[celli] += w*omegaVis*4.5*op;

        }
        else if (yPlus < 5)
        {
            scalar op = tanh(pow(yPlus/10, 1));
            scalar bf = 0.51;

            omega0[celli] +=
                pow(pow(omegaVis*(1 - op), bf) + pow(omegaLog*op, bf), 1/bf);
        }
        else if (yPlus < yPlusLam)
        {
            scalar op = tanh(pow(yPlus/10, 1));
            scalar bf = 0.48;
